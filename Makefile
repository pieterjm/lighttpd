...
define Package/lighttpd
  ...
  TITLE:=A flexible and lightweight web server with ws
endef
...
define Package/lighttpd-mod-websocket
  $(call Package/lighttpd/Default)
  DEPENDS:=lighttpd
...
ifneq ($(SDK)$(CONFIG_PACKAGE_lighttpd-mod-websocket),)
  CONFIGURE_ARGS+= \
	--with-websocket=ALL \
        --with-libicu=no
endif
...
define Package/lighttpd/install
	...
	$(INSTALL_DIR) $(1)/www
	$(INSTALL_DATA) ./files/mqtt.html $(1)/www/
	$(INSTALL_DIR) $(1)/www/js
	$(INSTALL_DATA) ./files/mqttws31.js $(1)/www/js/
...
$(eval $(call BuildPlugin,lighttpd-mod-websocket,websocket))
